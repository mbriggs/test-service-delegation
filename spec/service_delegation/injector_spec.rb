require '../../lib/service_delegation/injector'
require 'rack'

describe ServiceDelegation::Injector do
  ...
  let(:injector) do
    ServiceDelegation::Injector.new([ServiceDelegation::Injectables::Fixed.new(:id, 5)])
  end
  ...
  it "throws an error when no source is found for an argument" do
    ...
    end.to raise_error(UnknownInjectable)
  end

  it "throws an sane error when given no injectables" do
    klass = Class.new { def f(unknown_argument); end }
    expect do
      ServiceDelegation::Injector.new.call(klass.new.method(:f))
    end.to raise_error(UnknownInjectable)
  end
  ...
  context "custom injectables" do
    ...
    it "injects custom injectables" do
      ...
      injector = ServiceDelegation::Injector.for_app(app)
